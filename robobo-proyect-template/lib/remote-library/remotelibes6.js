...
export default class Remote {
  ...
  moveWheelsByDegree(wheel, degrees, speed) {
  ...
  moveWheelsByTime(wheel, time, speed) {
    const message = JSON.stringify({
        "name": "MOVEBYTIME",
        "parameters": {
            wheel,
  ...
  moveWheelsSeparated(lSpeed, rSpeed, time) {
  ...
  moveWheelsSeparatedWait(lSpeed, rSpeed, time, callback) {
  ...
  movePanByDegrees(degrees, speed) {
    ...
    let actual = this.statusmap.get("panPos");
    if (actual==undefined){
      actual = 180;
    }
    let newpos = parseInt(actual) + parseInt(degrees);
    if (newpos > 339){
      newpos = 339;
    }
    if (newpos < 27){
  ...
  moveTiltByDegrees(degrees, speed) {
    ...
    let actual = this.statusmap.get("tiltPos");
    if (actual==undefined){
      actual = 90;
    }
    let newpos = parseInt(actual) + parseInt(degrees);
      ...
      for (var key in msg.value) {
        //console.log(key);
          ...
          if (this.firstime){
            ...
            const now = parseInt(msg.value[key]);
            ...
            const then = this.laststatusmap.get(key);
